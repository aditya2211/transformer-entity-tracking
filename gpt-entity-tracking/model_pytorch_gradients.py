...
class TransformerModel(nn.Module):
    ...
    def forward(self, x, gradient_holder):
        x = x.view(-1, x.size(-2), x.size(-1))
        e = self.drop(self.embed(x))
        gradient_holder.e = e
        gradient_holder.e.retain_grad()
        gradient_holder.sum_embeds = self.embed.weight.sum(0)
        #ok.retain_grad()
...
class ClfHeadModified(nn.Module):
    """Classification Head for the transformer

    TODO: test this class."""
    def __init__(self, clf_token, extra_token, cfg, n_class):
        super(ClfHeadModified, self).__init__()
        self.n_embd = cfg.n_embd
        self.clf_token = clf_token
        self.extra_token = extra_token
        self.dropout = nn.Dropout(cfg.clf_pdrop)
        self.linear = nn.Linear(cfg.n_embd*2, n_class)

        nn.init.normal_(self.linear.weight, std = 0.02)
        nn.init.normal_(self.linear.bias, 0)

    def forward(self, h, h2, x, x2):
        clf_h = h.view(-1, self.n_embd)
        flat = x[..., 0].contiguous().view(-1)
        clf_h = clf_h[flat == self.clf_token, :]
        clf_h = self.dropout(clf_h)

        clf_h2 = h2.view(-1, self.n_embd)

        flat2 = x2[..., 0].contiguous().view(-1)


        clf_h2 = clf_h2[flat2 == self.extra_token, :]


        clf_h2 = self.dropout(clf_h2)
        clf_h2 = clf_h2.expand(clf_h.shape)

        clf_comb = torch.cat((clf_h, clf_h2), 1)

        clf_logits = self.linear(clf_comb)
...
class DoubleHeadModel(nn.Module):
    ...
    def __init__(self, cfg, clf_token, task_head_type, vocab=40990, n_ctx=512):
        ...
        self.extra_linear = nn.Linear(cfg.n_embd*2, 2)
        if isinstance(task_head_type, str):
            ...
            elif task_head_type == 'custom':
                self.task_head = ClfHead(clf_token, cfg, 2)

            elif task_head_type == 'propara':
                self.task_head = ClfHead(clf_token, cfg, 5)
            else:
...
class DoubleHeadModelwHidden(nn.Module):
    """ Transformer with language model and task specific heads """
    def __init__(self, cfg, clf_token, task_head_type, vocab=40990, n_ctx=512):
        super(DoubleHeadModelwHidden, self).__init__()
        self.transformer = TransformerModel(cfg, vocab=vocab, n_ctx=n_ctx)
        self.lm_head = LMHead(self.transformer, cfg)
        self.extra_linear = nn.Linear(cfg.n_embd*2, 2)
        if isinstance(task_head_type, str):
    ...
    def forward(self, x, gradient_holder):
        h,e = self.transformer(x, gradient_holder)
        lm_logits = self.lm_head(h)
        task_logits = self.task_head(h, x)

        return  h, e, lm_logits, task_logits

class DoubleHeadModelModified(nn.Module):
    """ Transformer with language model and task specific heads """
    def __init__(self, cfg, clf_token, extra_token, task_head_type, vocab=40990, n_ctx=512):
        super(DoubleHeadModelModified, self).__init__()
        self.transformer = TransformerModel(cfg, vocab=vocab, n_ctx=n_ctx)
        self.lm_head = LMHead(self.transformer, cfg)
        self.extra_linear = nn.Linear(cfg.n_embd*2, 2)
        if isinstance(task_head_type, str):
            if task_head_type == 'multiple_choice':
                ...
                self.task_head = ClfHeadModified(clf_token, extra_token, cfg, 2)
    ...
    def forward(self, x, x2):
        h = self.transformer(x)
        h2 = self.transformer(x2)

        lm_logits = self.lm_head(h)
        lm_logits2 = self.lm_head(h2)
        task_logits = self.task_head(h, h2, x, x2)

        return lm_logits, task_logits, lm_logits2
