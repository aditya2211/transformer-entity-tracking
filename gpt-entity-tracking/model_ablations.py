...
class ClfHeadModified(nn.Module):
    ...
    def __init__(self, clf_token, extra_token, cfg, n_class):
        ...
        self.dropout = nn.Dropout(cfg.clf_pdrop)
        self.linear = nn.Linear(cfg.n_embd, n_class)
        self.bilinear = nn.Linear(cfg.n_embd, cfg.n_embd)

        nn.init.normal_(self.linear.weight, std = 0.02)
        nn.init.normal_(self.linear.bias, 0)

    def forward(self, h, h2, x):
        clf_h = self.dropout(h)

        clf_h2 = h2.view(-1, self.n_embd)
        clf_h2 = self.dropout(clf_h2)

        a1 = self.bilinear(clf_h2)


        a1 = torch.mm(clf_h, torch.t(a1))

        a1 = F.softmax(a1,  0)

        contextual = torch.mm(torch.t(a1), clf_h)



        clf_logits = self.linear(contextual)
...
class DoubleHeadModelModified(nn.Module):
    ...
    def forward(self, x, x2, lens, lens2):
        h = self.transformer(x)
        lm_logits = self.lm_head(h)
        # print(x2[0,:10])
        ...
        # print(">>>>>")
        # print(lens)
        # print(self.transformer.embed(x2)[0,:10,:100])
        h2 = self.transformer.embed(x2[:,:lens[0]]).mean(dim=1)
        # print(h2.shape)
        # print(h2[0,:100])
        ...
        task_logits = self.task_head(h, h2, x)
